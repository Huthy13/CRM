...
#Account Methods
    def save_account(self, account: Account):
        ...
        if account.account_id is None:
            ...
            return self.db.cursor.lastrowid # Assuming db.add_account successfully commits and lastrowid is set
        else:
            ...
            self.db.update_account(account.account_id, account.name, account.phone, account.billing_address_id,
                                   account.shipping_address_id, account.is_billing_same_as_shipping(),
                                   account.website, account.description)
            return account.account_id # Return the existing ID for updates
    ...
    def import_accounts_from_csv(self, filepath: str):
        ...
        try:
            with open(filepath, 'r', newline='', encoding='utf-8') as csvfile:
                ...
                new_account_id = None # Initialize before try block
                try:
                    logging.debug(f"[Row {accounts_processed}] Attempting to save account: {account.name}")
                    new_account_id = self.save_account(account) # Store the returned ID

                    if new_account_id and isinstance(new_account_id, int) and new_account_id > 0:
                        accounts_imported += 1
                        logging.info(f"[Row {accounts_processed}] Successfully imported account: {account.name} with new ID: {new_account_id}")
                    else:
                        # This case handles if save_account returns None or an invalid ID without raising an exception
                        logging.error(f"[Row {accounts_processed}] Failed to save account {account.name}. Reason: save_account returned invalid ID '{new_account_id}'. Row data: {row}")
                        accounts_skipped += 1

                except Exception as e: # Catch potential DB errors during save_account call itself

...
class TestSalesLogic(unittest.TestCase):
    ...
    def test_convert_quote_to_invoice_success(self):
        ...
        mock_quote_item_data = [{"id": 10, "sales_document_id": mock_quote_id, "product_id": 100, "product_description": "Item 1", "quantity": 2.0, "unit_price": 100.0, "discount_percentage": 0.0, "line_total": 200.0}]
        new_invoice_id = 2

        self.mock_db_handler.get_sales_document_by_id.side_effect = [mock_quote_data, {"id": new_invoice_id, "document_number": "INV-001", "customer_id": mock_customer_id, "document_type": SalesDocumentType.INVOICE.value, "status": SalesDocumentStatus.INVOICE_DRAFT.value, "related_quote_id":mock_quote_id, "total_amount":220.0, "created_date":"d"}]
        self.mock_db_handler.get_items_for_sales_document.side_effect = [[{k:v for k,v in item.items()} for item in mock_quote_item_data], [{**item, "sales_document_id":new_invoice_id} for item in mock_quote_item_data]]
        ...
        with patch.object(self.sales_logic, '_generate_sales_document_number', return_value="INV-001") as mock_gen_num:
            invoice = self.sales_logic.convert_quote_to_invoice(mock_quote_id)
    ...
    def test_convert_quote_to_invoice_quote_not_found(self):
        self.mock_db_handler.get_sales_document_by_id.return_value = None
        with self.assertRaisesRegex(ValueError, "Quote with ID 999 not found."):
            self.sales_logic.convert_quote_to_invoice(999)

    def test_convert_quote_to_invoice_not_a_quote(self):
        ...
        with self.assertRaisesRegex(ValueError, "Document ID 1 is not a Quote."):
            self.sales_logic.convert_quote_to_invoice(1)

    def test_convert_quote_to_invoice_wrong_status(self):
        ...
        with self.assertRaisesRegex(ValueError, "Only accepted Quotes can be converted to Invoices. Current status: Quote Draft"):
            self.sales_logic.convert_quote_to_invoice(1)
